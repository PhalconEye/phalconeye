...
class Tools
{
     ...
     * @return Phalcon\Config
     */
    public static function getWebToolsConfig()
    {
        return self::$_di->getShared('webToolsConfig');
    }
	...
	public static function main($path)
	{
        ...
        $webToolsConfigPath = "app/config/webtools.ini";
        if (file_exists($webToolsConfigPath)) {
            $webToolsConfig = new \Phalcon\Config\Adapter\Ini($webToolsConfigPath);
        } else {
            $webToolsConfigPath = "app/config/webtools.php";
            if (file_exists($webToolsConfigPath)) {
                $webToolsConfig = require $webToolsConfigPath;
            } else {
                throw new \Phalcon\Exception('Configuration file for WebTools could not be loaded');
            }
        }
		...
		try {
			...
			$di->set('view', function() use ($path) {
				...
				$view->setViewsDir($path . '/Web/Tools/views/');
				return $view;
			});

			$di->set('config', $config);
			$di->set('webToolsConfig', $webToolsConfig);
