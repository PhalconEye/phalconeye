...
class Model
{
     ...
     * @var string
     */
    protected  static $_migrationPath = null;
     ...
     * @return    string
     */
    public static function generateEmpty($version)
    {
        $classVersion = preg_replace('/[^0-9A-Za-z]/', '', $version);
        $className = 'MigrateMigration_' . $classVersion;
        $classData = "use Engine\\Generator\\Migrations\\Model as Migration;
...
class " . $className . " extends Migration\n" .
     ...
     */
    public static function migrateFile($version, $filePath, $migrateType)
    {
        if (file_exists($filePath)) {
            ...
            if (class_exists($className)) {
                ...
                if (method_exists($migration, $migrateType)) {
                    $migration->$migrateType();
                    $afterMethod = 'after' . ucfirst($migrateType);
                    if (method_exists($migration, $afterMethod)) {
                        $migration->$afterMethod();
                    }
                }
            } else {
    ...
    public function runSqlFile($filepath){
        if (file_exists($filepath)) {
            self::$_connection->begin();
            self::$_connection->query(file_get_contents($filepath));
            self::$_connection->commit();
        }
    }
