...
{% block title %}{{ "Packages management - Plugins"|trans }}{% endblock %}
                    ...
                    <li {% if not package.isEnabled() %}class="disabled"{% endif %}>
                        ...
                        {% if not package.isSystem() %}
                            <div class="package_options">
                                {{ link_to(['for':'admin-packages-edit', 'type':package.getType(), 'name':package.getName(), 'return':'admin-packages-plugins'], 'Edit' | trans, 'class': 'btn btn-inverse') }}
                                ...
                                {% if package.isEnabled() %}
                                    {{ link_to(['for':'admin-packages-disable', 'type':package.getType(), 'name':package.getName(), 'return':'admin-packages-plugins'], 'Disable' | trans, 'class': 'btn btn-warning') }}
                                {% else %}
                                    {{ link_to(['for':'admin-packages-enable', 'type':package.getType(), 'name':package.getName(), 'return':'admin-packages-plugins'], 'Enable' | trans, 'class': 'btn btn-success') }}
                                {% endif %}
