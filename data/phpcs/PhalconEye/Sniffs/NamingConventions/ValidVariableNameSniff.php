...
class PhalconEye_Sniffs_NamingConventions_ValidVariableNameSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
    ...
    protected function processMemberVar(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $currentClass = $phpcsFile->findPrevious(array(T_CLASS), $stackPtr - 1);
        $extenedClass = false;
        if ($currentClass) {
            $extenedClass = $phpcsFile->findExtendedClassName($currentClass);
        }

        if (PHP_CodeSniffer::isCamelCaps($varName, false, $public, false) === false && $extenedClass !== 'AbstractModel') {
            $error = 'Variable "%s" is not in valid camel caps format';
            $data = array($varName);
