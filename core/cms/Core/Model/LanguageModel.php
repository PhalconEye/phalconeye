 ...
 *
 * @Source("languages")
 * @HasMany("id", "\Core\Model\LanguageTranslationModel", "language_id", {
 *  "alias": "LanguageTranslationModel"
 * })
 */
class LanguageModel extends AbstractModel
{
     ...
     *
     * @param DIBehavior $di   Dependency injection.
    ...
    public static function parseImportData($di, $data)
    {
         ...
         */
        $language = LanguageModel::findFirst(["language = '{$data['language']}' AND locale = '{$data['locale']}'"]);
        if (!$language) {
    ...
    {
        return $this->getRelated('LanguageTranslationModel', $arguments);
    }
    ...
    public function validation()
    {
        ...
        $isValid = !(bool)LanguageModel::findFirst($condition);
        if (!$isValid) {
            $this->appendMessage(
    ...
    public function toTranslationsArray(array $scope = [])
    {
        ...
        $result = LanguageTranslationModel::getBuilder()
            ->where(
                $whereCondition,
