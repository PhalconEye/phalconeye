    ...
    <script type="text/javascript">
        var removePackage = function (type, name) {
            if (confirm('{{ 'Are you really want to remove this package? Once removed, it can not be restored.' |i18n}}')) {
                window.location.href = '{{ url(['for':'backoffice-packages-uninstall', 'type':'%type%', 'name':'%name%', 'return':'backoffice-packages-widgets']) }}'.replace('%type%', type).replace('%name%', name);
            }
        }
    </script>
{% endblock %}
                    ...
                    <li {% if not package.enabled %}class="disabled"{% endif %}>
                        ...
                        {% if not package.is_system %}
                            <div class="package_options">
                                {{ link_to(['for':'backoffice-packages-edit', 'type':package.type, 'name':package.name, 'return':'backoffice-packages-widgets'], 'Edit' |i18n, 'class': 'btn btn-default') }}
                                ...
                                {% if package.enabled %}
                                    {{ link_to(['for':'backoffice-packages-disable', 'type':package.type, 'name':package.name, 'return':'backoffice-packages-widgets'], 'Disable' |i18n, 'class': 'btn btn-warning') }}
                                {% else %}
                                    {{ link_to(['for':'backoffice-packages-enable', 'type':package.type, 'name':package.name, 'return':'backoffice-packages-widgets'], 'Enable' |i18n, 'class': 'btn btn-success') }}
                                {% endif %}
