...
{% block head %}
    <script type="text/javascript">
        var requestAddItem = function () {
            var url = '{{ url(['for':'backoffice-languages-create-item'])}}';
            var data = {
                'language_id': {{ lang.id }}
            };
        ...
        var editItem = function (id) {
            var url = '{{ url(['for':'backoffice-languages-edit-item'])}}' + id;
            var data = {
                'id': id,
                'language_id': {{ lang.id }}
            };
...
{% block content %}
    <div class="span12">
        <div class="language_manage_header">
            <h3>
                <a href="{{ url(['for': 'backoffice-languages']) }}">{{ "Languages" |i18n }}</a> > {{ lang.name }}
            </h3>
            ...
            {% if (lang.language != constant('\Engine\Config::CONFIG_DEFAULT_LANGUAGE')) %}
                ...
                <a class="btn btn-info"
                   href='{{ url(['for': 'backoffice-languages-synchronize', 'id': lang.id]) }}'>{{ 'Synchronize'|i18n }}</a>
            {% endif %}
            <form class="navbar-search pull-right" method="GET"
                  action="{{ url(['for': 'backoffice-languages-manage'])~lang.id }}">
                {% if search is defined %}
                    <div class="glyphicon glyphicon-remove"
                         onclick="window.location.href='{{ url(['for': 'backoffice-languages-manage'])~lang.id }}'"></div>
                {% endif %}
